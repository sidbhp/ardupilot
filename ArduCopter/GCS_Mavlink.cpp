...
bool GCS_MAVLINK::try_send_message(enum ap_message id)
{
    ...
    case MSG_ARMMASK:
        mavlink_msg_named_value_int_send(chan, hal.scheduler->millis(), "ARMMASK", copter.get_ready_to_arm_mode_mask());
        break;
...
void GCS_MAVLINK::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_PARAM_VALUE:{
        if(msg->compid == MAV_COMP_ID_GIMBAL){
            copter.camera_mount._externalParameters.handle_param_value(&copter.DataFlash, msg);
        }
        break;
    }
    ...
    case MAVLINK_MSG_ID_COMMAND_LONG:       // MAV ID: 76
    {
        ...
        case MAV_CMD_DO_SEND_BANNER: {
            ...
            send_text_P(MAV_SEVERITY_INFO, PSTR(FIRMWARE_STRING));

            #if defined(PX4_GIT_VERSION) && defined(NUTTX_GIT_VERSION)
            send_text_P(MAV_SEVERITY_INFO, PSTR("PX4: " PX4_GIT_VERSION " NuttX: " NUTTX_GIT_VERSION));
            #endif

            send_text_P(MAV_SEVERITY_INFO, PSTR("Frame: " FRAME_CONFIG_STRING));

            #if CONFIG_HAL_BOARD != HAL_BOARD_APM1 && CONFIG_HAL_BOARD != HAL_BOARD_APM2
            // send system ID if we can
            char sysid[40];
            if (hal.util->get_system_id(sysid)) {
                send_text(MAV_SEVERITY_CRITICAL, sysid);
            }
            #endif
    ...
    {
        if (copter.accelcal.get_status() == ACCEL_CAL_WAITING_FOR_ORIENTATION) {
            copter.accelcal.collect_sample();
        }
