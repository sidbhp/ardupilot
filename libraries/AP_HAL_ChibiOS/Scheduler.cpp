...
void Scheduler::get_stats(void)
{
    ...
    for (uint8_t i=0; i<idx; i++) {
        ...
        hal.console->printf("%12s %4u %12s %2u%% %10u %6u %6u %8u %6u\n", names[i], (unsigned)prio[i], state_names[states[i]], (unsigned)pct, (unsigned)task_usec[i], (unsigned)num_runs[i], (unsigned)(task_usec[i]/num_runs[i]), (unsigned)max_usec[i], (unsigned)min_usec[i]);
    }
    
    _busy_percent = 100 * (1.0f - (float)idle_time / total_time);

    hal.console->printf("busy percent: %d, n_irq: %lu total=%u\n", (unsigned)_busy_percent, ch.kernel_stats.n_irq, (unsigned)total_time);
}
