...
int CAN::openSocket(const std::string& iface_name)
{
    ...
    if (s < 0)
    {
        printf("tried making a socket and failed :(\n");
        return s;
    }
    ...
    int res = ioctl(s, SIOCGIFINDEX, &ifr);
    printf("res: %d\n", res);
    if (res < 0 || ifr.ifr_ifindex < 0)
    {
        printf("???\n");
        return -1;
    }
...
void CAN::_pollWrite()
{
    while (hasReadyTx())
    {
        ...
        if (tx.deadline >= getMonotonic())
        {
            ...
            {          // Not transmitted, nor is it an error
...
void CAN::_pollRead()
{
    ...
    while (iterations_count < CAN_MAX_POLL_ITERATIONS_COUNT)
    {
        ...
        if (res == 1)
        {
            bool accept = true;
            if (loopback)
            { // We receive loopback for all CAN frames
                _confirmSentFrame();
...
int CAN::_write(const uavcan::CanFrame& frame) const
{
    ...
    if (res <= 0)
    {
        ...
        { // Writing is not possible atm, not an error
            return 0;
        }
        return res;
    }
...
{
    int res = init(can_number);
    if (/*init(can_number)*/res >= 0)
    {
        _initialized = true;
        printf("OMG\n");
    }
...
{
    printf("\ninitializing SITL CANManager\n");
    int res = -1;
    char iface_name[16];
    //#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    //sprintf(iface_name, "vcan%u", can_number);
    //#else
    sprintf(iface_name, "vcan%u", can_number);
    //#endif
    printf("name: %s\n", iface_name);
...
int CANManager::addIface(const std::string& iface_name)
{
    ...
    if (fd < 0)
    {
        printf("tried opening the socket and died :(\n");
        return fd;
    }
