...
def configure(cfg):
    ...
    env.WOLFSSL_BUILDDIR = bldpath('modules/wolfssl')
...
def build(bld):
    if bld.env.TOOLCHAIN == 'native':
        ...
        wolfssl_task = bld(
            ...
            rule="cd ${WOLFSSL_ROOT} && ./configure --prefix=${WOLFSSL_BUILDDIR} CFLAGS=\"${EXTRA_CFLAGS} -DWOLFSSL_USER_SETTINGS -I${WOLFSSL_USER_HEADERS} -Wno-error=unused-parameter\" --disable-filesystem --disable-fastmath --disable-shared --enable-sha --enable-keygen --enable-rsa --disable-oldnames && make clean && make && make install",
            group='dynamic_sources',
            source=wolfssl_source,
            target=bld.bldnode.find_or_declare('modules/wolfssl/lib/libwolfssl.a')
        )
    elif bld.env.TOOLCHAIN == 'arm-none-eabi':
        wolfssl_source = [bld.path.find_node('libraries/AP_HAL_ChibiOS/hwdef/common/wolfssl/user_settings.h')]
        wolfssl_task = bld(
            #build libwolfssl.a from WolfSSL sources
            rule="cd ${WOLFSSL_ROOT} && ./configure --host=arm-none-eabi CC=arm-none-eabi-gcc AR=arm-none-eabi-ar STRIP=arm-none-eabi-strip RANLIB=arm-none-eabi-ranlib --prefix=${WOLFSSL_BUILDDIR} CFLAGS=\"--specs=nosys.specs -mthumb ${EXTRA_CFLAGS} -DWOLFSSL_USER_SETTINGS -I${WOLFSSL_USER_HEADERS} -Wno-error=unused-parameter\" --disable-filesystem --disable-fastmath --disable-shared --enable-sha --enable-keygen --enable-rsa --disable-oldnames && make clean && make && make install",
            group='dynamic_sources',
            source=wolfssl_source,
            target=bld.bldnode.find_or_declare('modules/wolfssl/lib/libwolfssl.a')
        )
    ...
    bld.env.LIB += ['wolfssl']
    bld.env.LIBPATH += ['modules/wolfssl/lib']
    bld.env.INCLUDES += ['modules/wolfssl/include']
